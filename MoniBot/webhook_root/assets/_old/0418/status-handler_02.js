...
class StatusHandler {
    ...
    setupMuteToggle() {
        ...
        if (muteButton) {
            ...
            muteButton.addEventListener('click', () => {
                ...
                // 通知ログのみ（ポップアップなし）
                console.log(newMuteState ? 'ミュートにしました' : 'オンにしました');
            });
        }
    }
    ...
    updateBacklogStatus(isPresent) {
        ...
        if (!statusId) {
            ...
            console.log('警告: ステータスの更新ができませんでした。ステータスIDが不明です。');
            return;
        }
        ...
        .catch(error => {
            ...
            // エラーログのみ（ポップアップなし）
            console.log(`ステータス更新に失敗しました: ${error.message}`);
        });
    }
    ...
    processTicketEvent(event) {
        ...
        if (event.data && event.data.assigneeId && event.data.assigneeId.toString() === this.userId) {
            ...
            // 通知音のみ再生（ミュート設定に従う）- ポップアップは表示しない
    ...
    completeTicket(ticketId) {
        ...
        if (!statusIdCompleted) {
            ...
            console.log('エラー: 完了ステータスIDが見つかりません');
            return;
        }
        ...
        .then(response => {
            ...
            // 完了ログのみ（ポップアップなし）
            console.log(`チケットを完了しました: チケットID ${ticketId}`);
        })
        .catch(error => {
            console.error('チケット完了エラー:', error);
            console.log('エラー: チケットの完了処理に失敗しました');
        });
    }
    ...
    returnTicket(ticketId) {
        ...
        if (!statusIdReturn || !statusIdAbsent) {
            ...
            console.log('エラー: 差戻または不在ステータスIDが見つかりません');
            return;
        }
        ...
        .then(() => {
            ...
            // 差戻しログのみ（ポップアップなし）
            console.log(`チケットを差戻しました: チケットID ${ticketId}`);
        })
        .catch(error => {
            console.error('チケット差戻エラー:', error);
            this.hideProcessingModal();
            console.log('エラー: 差戻処理中にエラーが発生しました');
        });
    }
    ...
    refreshTickets() {
        ...
        fetch(`${apiBaseUrl}/issues?${params.toString()}`)
            ...
            .catch(error => {
                ...
                console.log('エラー: チケット情報の取得に失敗しました');
            });
    }
